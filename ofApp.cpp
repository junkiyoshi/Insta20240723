...
void ofApp::update() {
	...
	int radius_max = this->base_radius * 8;
	for (auto& triangle : this->ico_sphere.getMesh().getUniqueFaces()) {
		...
		int radius_end = noise_value > 0.65 ? ofMap(noise_value, 0.65, 1, radius_start, radius_max) : radius_start;
		...
		for (int radius = radius_start; radius <= radius_end; radius += 1) {
			...
			auto value = ofMap(radius, radius_start, radius_max, 39, 255);
